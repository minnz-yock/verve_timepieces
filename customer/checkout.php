...
// Load saved addresses for prefills
function fetch_address(PDO $conn, int $userId, string $type) {
  $st = $conn->prepare('SELECT first_name, last_name, country_region, street_address, address_line2, city_town, phone FROM addresses WHERE user_id = ? AND address_type = ?');
  $st->execute([$userId, $type]);
  return $st->fetch(PDO::FETCH_ASSOC) ?: [];
}

$shippingSaved = fetch_address($conn, $userId, 'shipping');
$billingSaved  = fetch_address($conn, $userId, 'billing');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  if ($step === 'shipping') {
    ...
    if ($isMm) {
      ...
      if ($s['method'] === 'express_intl') { $s['cost'] = 50.0; $s['method'] = 'Express Intl (3–7 days, DHL/UPS)'; }
      else { $s['cost'] = 20.0; $s['method'] = 'Standard Intl (1–3 weeks)'; }
    }
    header('Location: checkout.php?step=billing'); exit;
  }
  if ($step === 'billing') {
    $b =& $state['billing'];
    ...
    header('Location: checkout.php?step=review'); exit;
  }
}
...
<head>
  ...
  <style>
    body { color:#352826; }
    .container-narrow{ max-width:980px; margin: 0 auto; padding: 16px 12px 40px; }
    .steps{ display:flex; gap:20px; align-items:center; justify-content:center; margin: 8px 0 20px; }
    .step{ display:flex; align-items:center; gap:8px; color:#785A49; }
    .step .dot{ width:28px; height:28px; border-radius:50%; display:grid; place-items:center; border:1px solid #DED2C8; }
    .step.active .dot{ background:#352826; color:#fff; border-color:#352826 }
    .card{ border:1px solid #DED2C8; }
    .card-header{ font-weight:800; color:#352826 }
    .form-control, .form-select{ border-color:#A57A5B; color:#352826 }
    .form-control:focus, .form-select:focus{ border-color:#785A49; box-shadow:0 0 0 .2rem rgba(120,90,73,.15) }
    .btn-dark{ background:#352826; border-color:#352826 }
    .btn-dark:hover{ background:#785A49; border-color:#785A49 }
    .btn-outline-dark{ color:#352826; border-color:#352826 }
    .btn-outline-dark:hover{ background:#352826; color:#fff }
    .summary-card{ border:1px solid #DED2C8; border-radius:8px; }
    .img-thumb{ width:48px; height:48px; border:1px solid #DED2C8; border-radius:6px; display:grid; place-items:center; overflow:hidden; background:#fff }
  </style>
</head>
<body>
  ...
  <div class="container-narrow">
    ...
    <div class="steps">
      <div class="step <?= $step==='shipping'?'active':'' ?>"><div class="dot">1</div> <div>Shipping Address</div></div>
      <div class="step <?= $step==='billing'?'active':'' ?>"><div class="dot">2</div> <div>Billing & Payment</div></div>
      <div class="step <?= $step==='review'?'active':'' ?>"><div class="dot">3</div> <div>Order Review</div></div>
    </div>

    <?php if ($step === 'shipping'): $s=$state['shipping']; $isMm = strcasecmp($s['country_region'], 'Myanmar')===0; ?>
      <div class="card">
    ...
    <?php else: $s=$state['shipping']; $b=$state['billing']; $subtotal=0; foreach($cartItems as $it){ $subtotal += ((float)$it['price']*(int)$it['quantity']); } $total=$subtotal + (float)$s['cost']; ?>
      <div class="row g-3">
  ...
  <script>
    ...
    (function(){
      ...
      function refresh(){
        ...
        document.querySelectorAll('.mm-only').forEach(function(el){ el.style.display = isMm? 'block':'none'; });
        document.querySelectorAll('.intl-only').forEach(function(el){ el.style.display = isMm? 'none':'block'; });
      }
    ...
    (function(){
      ...
      same.addEventListener('change', function(){ fields.classList.toggle('d-none', same.checked); });
    })();
  </script>
</body>
</html>
